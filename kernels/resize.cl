...
__kernel void nearest2D(__read_only image2d_t src, __write_only image2d_t dst) {
  ...
  const float2 rescale_ratio =
      convert_float2(get_image_dim(src)) / convert_float2(get_image_dim(dst));
...
__kernel void linear2D(__read_only image2d_t src, __write_only image2d_t dst) {
  ...
  const float4 pixY0 = mix(
      convert_float4_rtz(read_imageui(src, sampler, read_pos + (int2)(0, 0))),
      convert_float4_rtz(read_imageui(src, sampler, read_pos + (int2)(1, 0))),
      sample_ratio.x);
  const float4 pixY1 = mix(
      convert_float4_rtz(read_imageui(src, sampler, read_pos + (int2)(0, 1))),
      convert_float4_rtz(read_imageui(src, sampler, read_pos + (int2)(1, 1))),
      sample_ratio.x);

  // Interpolating along Y
  const uint4 pix =
      convert_uint4_sat_rtz(mix(pixY0, pixY1, sample_ratio.y) + 0.5f);
...
__kernel void nearest3D(__read_only image3d_t src, __write_only image3d_t dst) {
  ...
  const sampler_t sampler = CLK_NORMALIZED_COORDS_FALSE |
                            CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST;
  const float4 rescale_ratio =
      convert_float4(get_image_dim(src)) / convert_float4(get_image_dim(dst));
...
__kernel void linear3D(__read_only image3d_t src, __write_only image3d_t dst) {
  ...
  // Interpolating along X
  const float4 pix000_100 =
      mix(convert_float4_rtz(
              read_imageui(src, sampler, read_pos + (int4)(0, 0, 0, 0))),
          convert_float4_rtz(
              read_imageui(src, sampler, read_pos + (int4)(1, 0, 0, 0))),
          sample_ratio.x);
  const float4 pix010_110 =
      mix(convert_float4_rtz(
              read_imageui(src, sampler, read_pos + (int4)(0, 1, 0, 0))),
          convert_float4_rtz(
              read_imageui(src, sampler, read_pos + (int4)(1, 1, 0, 0))),
          sample_ratio.x);
  const float4 pix001_101 =
      mix(convert_float4_rtz(
              read_imageui(src, sampler, read_pos + (int4)(0, 0, 1, 0))),
          convert_float4_rtz(
              read_imageui(src, sampler, read_pos + (int4)(1, 0, 1, 0))),
          sample_ratio.x);
  const float4 pix011_111 =
      mix(convert_float4_rtz(
              read_imageui(src, sampler, read_pos + (int4)(0, 1, 1, 0))),
          convert_float4_rtz(
              read_imageui(src, sampler, read_pos + (int4)(1, 1, 1, 0))),
          sample_ratio.x);
  ...
  const uint4 pix =
      convert_uint4_sat_rtz(mix(pixZ0, pixZ1, sample_ratio.z) + 0.5f);
